	...
	<data-tables :data.sync="data" ref="table" :table-props="tableProps" :filters="filters" @row-click="clickThenSelect"
		...
		<el-table-column v-for="title in titles" :key="title.prop" v-bind="title">
			<template slot-scope="scope">
				...
				<span v-else-if="title.prop === 'role'">{{ roleFormatter(scope.row[title.prop]) }}</span>
	...
	export default {
		...
		methods: {
			...
			rowOpt(opt, id) {
				...
				const cfm = '是否' + labelMsg[opt] + '用户' + (id === undefined ? '(' + this.multiSelection.length + ')' : '');
				this.$confirm(cfm, '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
				}).then(() => {
